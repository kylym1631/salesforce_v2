...
{{--    <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">--}}
        <div class="sm:col-span-2"
             x-data="{ show: {{ session()->has('success_message') ? 'true' : 'false' }} }"
        >

            <div class="rounded-md bg-green-50 p-4"
            x-show.transition="show"
                 x-init="setTimeout(() => { show = false; }, 4000)"
                 x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-500" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-90"
            >
                <div class="flex">
